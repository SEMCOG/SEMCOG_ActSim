...
if(!("rFreight" %in% installed.packages(lib.loc = SYSTEM_PKGS_PATH)[,1])){
  rFreightZipName <- list.files(dirname(SYSTEM_RFREIGHT_PATH),
                                pattern = "rFreight")
  if(length(rFreightZipName)<1){
    stop(paste("rFreight package zip file not available at", 
               dirname(SYSTEM_RFREIGHT_PATH)))
    ...
    install.packages(file.path(dirname(SYSTEM_RFREIGHT_PATH), rFreightZipName),
