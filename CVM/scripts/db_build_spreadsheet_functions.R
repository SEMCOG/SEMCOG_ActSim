...
createTAZSummaries <- function(scenario, scenario_output_path, load_db_workspace = FALSE){ 
  ...
  # Trip tables: trips
  tt_taz_o_trips <- TripTable[,.(NumTrips = sum(trips, na.rm = TRUE)),
                               keyby = .(TAZ = OTAZ, Vehicle)]
  
  tt_taz_d_trips <- TripTable[,.(NumTrips = sum(trips, na.rm = TRUE)),
...
createAssignmentSummaries <- function(scenario, model_flows){
  ...
  model_flows[, TOT_FLOW_PASS_CAR := AB_FLOW_DRIVEALONE + BA_FLOW_DRIVEALONE + AB_FLOW_SHARED2 + BA_FLOW_SHARED2 + AB_FLOW_SHARED3 + BA_FLOW_SHARED3]
  ...
  # any counts on links not present in the flow outputs will be filtered out
