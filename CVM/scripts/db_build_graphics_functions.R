...
addTAZPoints <- function(map, data, group, shp, TAZcol = "TAZ", scale.factor = 1,
  ...
  for (level in layers) {
    ...
    for (i in seq(1, nrow(counts))) {
      ...
      tryCatch(
        {
          points <- st_coordinates(st_sample(shp[shp$TAZ == counts[i, get(TAZcol)], ],
                                             size = n, type = "random"))
...
prepTAZPolygons <- function(shp, group.by) {
  ...
  # choice in the data frame of an sf Simple feature collection object.
  #
  # Args:
  #   shp      : Simple feature collection object with polygons for each zone
  ...
  #   shp      : Simple feature collection object with aggregated polygons
  ...
  colorFun <- colorFactor(palette = "Set3", domain = TAZ.polys$Group)
