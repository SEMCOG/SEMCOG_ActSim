...
db_build_render <- function() {
  ...
  dashboardName <- paste0("CVM_ReportDashboard_", currentDate, ".html")
  
  # Generate dashboard and save to root of scenario output directory
  ...
  dashboardHTML <- readLines(file.path(SCENARIO_OUTPUT_DIR, dashboardName))
  ...
  writeLines(dashboardHTML, file.path(SCENARIO_OUTPUT_DIR, dashboardName))
