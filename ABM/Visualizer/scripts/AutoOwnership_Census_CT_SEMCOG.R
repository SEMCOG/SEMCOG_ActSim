...
#Parameters_File <- "E:/Projects/Clients/SEMCOG/Tasks/Task19_Visualizer_Half/runtime/parameters.csv"
...
.libPaths(R_LIBRARY)
LOAD_PKGS_LIST <- c("leaflet", "htmlwidgets", "sf", "raster", "dplyr", "readr")
...
setwd(ZONES_DIR)
taz_shp <- st_read(SHP_FILE_NAME)
ct_shp <- st_read(ct_shp_file)

setwd(ABM_SUMMARY_DIR) # output dir

# Need Geographic crosswalk between taz and census tract
taz_shp <- st_transform(taz_shp, crs = st_crs("+proj=longlat +ellps=GRS80"))
ct_shp <- st_transform(ct_shp, crs = st_crs("+proj=longlat +ellps=GRS80"))

taz_centroids <- st_centroid(taz_shp)
tazXWalk <- st_join(taz_centroids, ct_shp)
...
zero_auto_hh_by_taz$CTIDFP <- tazXWalk$GEOID[match(zero_auto_hh_by_taz$TAZ, tazXWalk$ID)]
zero_auto_hh_by_taz$CTIDFP <- as.numeric(zero_auto_hh_by_taz$CTIDFP)
zero_auto_hh_by_taz$COUNTYFP <- tazXWalk$COUNTYFP[match(zero_auto_hh_by_taz$TAZ, tazXWalk$ID)]
...
ct_shp <- merge(ct_shp, df, 
                by.x = "GEOID", by.y ="TractID",
                all.x = TRUE)
ct_shp[is.na(ct_shp)] <- 0

# Create Map
ct_shp <- ct_shp[!is.na(ct_shp$Diff_ZeroAuto),]
#ct_shp <- ct_shp[!is.na(ct_shp$Diff_ZeroAuto),]
ct_shp$textComment1 <- paste("Total Census HH: ", ct_shp$Census_HH, sep = "")
ct_shp$textComment2 <- ifelse(ct_shp$Diff_ZeroAuto<0,'Model under predicts by',
                             ifelse(ct_shp$Diff_ZeroAuto==0,"Model correct",'Model over predicts by'))

st_write(ct_shp, file.path(ABM_SUMMARY_DIR, CT_ZERO_AUTO_FILE_NAME), append = FALSE)
