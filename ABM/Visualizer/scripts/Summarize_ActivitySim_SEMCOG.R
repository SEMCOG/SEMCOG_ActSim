###################################################################################
### Script for summarizing SEMCOG ActivitySim Output 
### Author: David Hensle, david.hensle@rsginc.com,
### Based on SummarizeABM_county_HHwgt.R script for CTRAMP outputs by Binny M. Paul
### Edited: DH, April 2019
###################################################################################
...
Run_switch          <- args[2]
#Parameters_File <- "C:/T6_ActSim_Test/ABM/Visualizer/runtime/parameters.csv"
...
.libPaths(R_LIBRARY)
# list of packages, including rFreight for omx functions
SYSTEM_REPORT_PKGS <- c("data.table", "plyr", "weights", "reshape", "stringr", "readr", "rFreight", "rhdf5")
#library(omxr)
lib_sink <- suppressWarnings(suppressMessages(lapply(SYSTEM_REPORT_PKGS, library, character.only = TRUE))) 

WD <- ABM_SUMMARY_DIR

xwalk <- read.csv(paste(ZONES_DIR, "Zones_xwalk.csv", sep = "/"), as.is = T)
# skim_file = file.path(SKIMS_DIR, "DIST.omx")
skim_file = file.path(SKIMS_DIR, "skims.omx")

setwd(ABM_DIR)
hh                 <- read_csv("final_households.csv")
per                <- read_csv("final_persons.csv")
all_tours          <- read_csv("final_tours.csv")
all_trips          <- read_csv("final_trips.csv")
jtour_participants <- read_csv("final_joint_tour_participants.csv")
#2zone maz to taz conversion reference
...
modeCodes <- data.frame(code = c(1, 2, 3, 4, 5, 6, 6, 6, 7, 7, 7, 8, 8, 8, 9, 10, 10, 10),
                        name = c("DRIVEALONE", "SHARED2", "SHARED3", "WALK", "BIKE",
                                 "WALK_LOC", "WALK_MIX", "WALK_PRM", # 6 = WT
                                 "PNR_LOC", "PNR_MIX", "PNR_PRM",    # 7 = PNR
                                 "KNR_LOC", "KNR_MIX", "KNR_PRM",    # 8 = KNR
...
xwalk$COUNTY_NAME <- countynames$name[match(xwalk$COUNTY, countynames$code)]
xwalk$TAZ <- xwalk$ID
...
skimMat_MD <- readMatrixOMX(skim_file, "SOV_DIST__MD")
...
print('Prepare files for computing summary statistics')
...
# switching maz to taz for 2-zone 
per$home_zone_id_taz <- taz_maz$TAZ[match(per$home_zone_id, taz_maz$zone_id)]
per$workplace_zone_id_taz <- taz_maz$TAZ[match(per$workplace_zone_id, taz_maz$zone_id)]

per$HDISTRICT <- xwalk$COUNTY_NAME[match(per$home_zone_id_taz, xwalk$TAZ)]
per$WDISTRICT <- xwalk$COUNTY_NAME[match(per$workplace_zone_id_taz, xwalk$TAZ)]
...
# switching maz to taz for 2-zone 
hh$home_zone_id_taz <- taz_maz$TAZ[match(hh$home_zone_id, taz_maz$zone_id)]


hh$COUNTYFP10 <- xwalk$COUNTY[match(hh$home_zone_id_taz, xwalk$TAZ)]
autoOwnershipCY <- count(hh, c("COUNTYFP10", "HHVEH"), "finalweight")
...
# switching maz to taz for 2-zone 
all_tours$OTAZ <- taz_maz$TAZ[match(all_tours$origin, taz_maz$zone_id)]
all_tours$DTAZ <- taz_maz$TAZ[match(all_tours$destination, taz_maz$zone_id)]
...
# No trip mode field - copy tour mode
if(any(grepl("trip_mode", names(all_trips)))){
  all_trips$TRIPMODE <- modeCodes$code[match(all_trips$trip_mode, modeCodes$name)]
} else {
  all_trips$TRIPMODE <- all_trips$TOURMODE
}
...
#2-zone
all_trips$OTAZ<- taz_maz$TAZ[match(all_trips$origin, taz_maz$zone_id)]
all_trips$DTAZ <- taz_maz$TAZ[match(all_trips$destination, taz_maz$zone_id)]
...
# No depart field - insert 1s 
if(any(grepl("depart", names(all_trips)))){
  all_trips$depart_hour <- all_trips$depart
} else {
  all_trips$depart_hour <- 1
}
...
all_trips$inbound <- ifelse(as.character(all_trips$outbound) %in% c("false", "FALSE"), 1, 0)
all_trips$inb_next[1:(nr-1)] <- all_trips$inbound[2:nr]
all_trips$next_tour_id <- 0
all_trips$next_tour_id[1:(nr-1)] <- all_trips$tour_id[2:nr]
all_trips$stops[all_trips$DPURP>0 & ((all_trips$inbound==0 & all_trips$inb_next==0) | (all_trips$inbound==1 & all_trips$inb_next==1)) & (all_trips$tour_id == all_trips$next_tour_id)] <- 1
...
# Creating individual columns for up to 20 persons in each joint tour: PER1, PER2,... and PTYPE1, PTYPE2,....
max_participant_num <- max(jtour_participants$participant_num)

participant_pivot <- dcast(as.data.table(jtour_participants), tour_id ~ participant_num, value.var = "person_id")
...
unique_joint_tours$NUMBER_HH <- (unique_joint_tours$PER1>0) + 
  ...
  (unique_joint_tours$PER13>0) + 
  (unique_joint_tours$PER14>0) + 
  (unique_joint_tours$PER15>0) + 
  (unique_joint_tours$PER16>0) + 
  (unique_joint_tours$PER17>0) + 
  (unique_joint_tours$PER18>0) + 
  (unique_joint_tours$PER19>0) + 
  (unique_joint_tours$PER20>0)


#create person level file for joint tours
jtours_per <- melt(unique_joint_tours[,c("household_id","tour_id", "PER1","PER2","PER3","PER4","PER5","PER6","PER7","PER8", "PER9", "PER10", "PER11", "PER12", "PER13", "PER14", "PER15", "PER16", "PER17", "PER18", "PER19", "PER20")], 
...
total_workers <- sum(per$finalweight[per$is_worker == TRUE])
total_employment<- sum(taz_maz$tot_emp)
...
total_pmt <- sum(trips$od_dist) + sum(jtrips$od_dist*jtrips$numpart_wgt)
cat("\n Total PMT : ", total_pmt)

total_vtrips <- sum(trips$finalweight[(trips$auto==1)]/trips$num_travel[(trips$auto==1)]) + sum(jtrips$numpart_wgt[(jtrips$auto==1)]/jtrips$num_travel[(jtrips$auto==1)])
cat("\n Total Vehicle Trips : ", total_vtrips)

total_vmt <- sum(trips$od_dist[(trips$auto==1)]/trips$num_travel[(trips$auto==1)]) + 
	sum((jtrips$od_dist[(jtrips$auto==1)]*jtrips$numpart_wgt[(jtrips$auto==1)])/jtrips$num_travel[(jtrips$auto==1)])
cat("\n Total VMT : ", total_vmt)

totals_var <- c("total_population", "total_households", "total_workers", "total_employment", "total_tours", "total_trips", "total_stops", "total_pmt", "total_vtrips", "total_vmt")
totals_val <- c(total_population,total_households, total_workers, total_employment, total_tours, total_trips, total_stops, total_pmt, total_vtrips, total_vmt)
...
hhSizeDist <- count(hh, c("HHSIZE"), "finalweight")
...
#transit pass and subsidy targets combine nonworker with retiree
per$PERTYPE_7 <- per$PERTYPE
per$PERTYPE_7[per$PERTYPE_7==5] <- 4

### Transit Pass and Transit Subsidy Models ###
# Percent that each persontype selects to have a transit pass
percent_pass_holders <- ddply(per[,c("PERTYPE_7", "transit_pass_ownership", "finalweight")], c("PERTYPE_7"), summarise, percent_pass_holders = weighted.mean(transit_pass_ownership, finalweight)*100)
write.table(percent_pass_holders, "percentPassHolders.csv", sep=",", row.names=F)


# Percent that each persontype has a transit subsidy
percent_with_subsidy <- ddply(per[,c("PERTYPE_7", "transit_pass_subsidy", "finalweight")], c("PERTYPE_7"), summarise, percent_with_subsidy = weighted.mean(transit_pass_subsidy, finalweight)*100)
write.table(percent_with_subsidy, "pecentWithSubsidy.csv", sep=",", row.names=F)

# Transit subsidy can change depending on tour type. Calculating average transit subsidy percent across all tours for each person.
tours$fare_discount_percent <- 0
person_average_subsidy <- ddply(tours[,c("person_id", "fare_discount_percent", "finalweight")], c("person_id"), summarise, person_average_subsidy = weighted.mean(fare_discount_percent, finalweight)*100)
per$transitSubsidy_percent <- person_average_subsidy$person_average_subsidy[match(per$person_id, person_average_subsidy$person_id)]

# Average subsidy amount if the subsidy exists
average_subsidy_amount <- ddply(per[per$transit_pass_subsidy == 1,c("PERTYPE_7", "transitSubsidy_percent", "finalweight")], c("PERTYPE_7"), summarise, average_subsidy_amount = weighted.mean(transitSubsidy_percent, finalweight)*100)
write.table(average_subsidy_amount, "averageSubsidyAmount.csv", sep=",", row.names=F)

# Subsidy Distribution by Person Type
subsidyfreq1 <- if (sum(per$PERTYPE==1 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==1 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==1 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq2 <- if (sum(per$PERTYPE==2 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==2 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==2 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq3 <- if (sum(per$PERTYPE==3 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==3 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==3 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq4 <- if (sum(per$PERTYPE==4 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==4 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==4 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq5 <- if (sum(per$PERTYPE==5 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==5 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==5 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq6 <- if (sum(per$PERTYPE==6 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==6 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==6 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq7 <- if (sum(per$PERTYPE==7 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==7 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==7 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq8 <- if (sum(per$PERTYPE==8 & per$transit_pass_subsidy==1) > 0) {
  wtd.hist(per$transitSubsidy_percent[per$PERTYPE==8 & per$transit_pass_subsidy==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==8 & per$transit_pass_subsidy==1])
} else {
  list(counts = rep(0, 11))
}

subsidyfreq <- data.frame(subsidyfreq1$counts, subsidyfreq2$counts, subsidyfreq3$counts, subsidyfreq4$counts, subsidyfreq5$counts
                          , subsidyfreq6$counts, subsidyfreq7$counts, subsidyfreq8$counts)
colnames(subsidyfreq) <- c("1", "2", "3", "4","5", "6", "7", "8")
write.csv(subsidyfreq, "transitSubsidyDC.csv")

# prepare stop location input for visualizer
subsidyDC_vis <- subsidyfreq
subsidyDC_vis$id <- row.names(subsidyDC_vis)
subsidyDC_vis <- melt(subsidyDC_vis, id = c("id"))
colnames(subsidyDC_vis) <- c("id", "PERTYPE", "freq")

subsidyDC_vis <- xtabs(freq~id+PERTYPE, subsidyDC_vis)
subsidyDC_vis <- addmargins(as.table(subsidyDC_vis))
subsidyDC_vis <- as.data.frame.matrix(subsidyDC_vis)
subsidyDC_vis$id <- row.names(subsidyDC_vis)
subsidyDC_vis <- melt(subsidyDC_vis, id = c("id"))
colnames(subsidyDC_vis) <- c("discountbin", "PERTYPE", "freq")
subsidyDC_vis$PERTYPE <- as.character(subsidyDC_vis$PERTYPE)
subsidyDC_vis$discountbin <- as.character(subsidyDC_vis$discountbin)
subsidyDC_vis <- subsidyDC_vis[subsidyDC_vis$discountbin!="Sum",]
subsidyDC_vis$PERTYPE[subsidyDC_vis$PERTYPE=="Sum"] <- "Total"
subsidyDC_vis$discountbin <- as.numeric(subsidyDC_vis$discountbin)

write.csv(subsidyDC_vis, "subsidyDC_vis.csv", row.names = F)
