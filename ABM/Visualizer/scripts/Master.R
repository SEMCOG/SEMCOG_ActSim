...
#Parameters_File <- "C:/Projects/SEMCOG/ABM/Visualizer/runtime/parameters.csv"
#Run_switch          <- "FULL"
...
Sys.setenv(
  PATH = paste(
    Sys.getenv("RSTUDIO_PANDOC"), RSTUDIO_PANDOC_path, 
...
source(paste(SCRIPTS_DIR, "scripts/_SYSTEM_VARIABLES.R", sep = "/"))

### Copy summary CSVs
base_csv_list <- ifelse(IS_BASE_SURVEY=="Yes", "summaryFilesNames_survey_SEMCOG.csv", "summaryFilesNames.csv")
...
#base_csv_list <- "summaryFilesNames_survey_SEMCOG.csv"
...
census_csv_list <- "summaryFilesNames_census_SEMCOG.csv"
...
retVal <- copyFile(summaryFileList_census, sourceDir = CENSUS_SUMMARY_DIR, targetDir = BASE_DATA_PATH)
if(retVal) q(save = "no", status = 11)
...
#jpeg_copy_retval <- file.copy(jpeg_list, BUILD_JPEG_PATH, overwrite = TRUE)

### Load required libraries
SYSTEM_REPORT_PKGS <- c("DT", "flexdashboard", "leaflet", "htmltools", "htmlwidgets", "kableExtra", "shiny",
                        "knitr", "mapview", "plotly", "RColorBrewer", "sf", "crosstalk","treemap", "htmlTable",
...
zone_shp <- st_read(SHP_FILE_NAME)
zone_shp <- st_transform(zone_shp, crs = st_crs("+proj=longlat +ellps=GRS80"))

setwd(ABM_SUMMARY_DIR)
ct_zero_auto_shp <- st_read(CT_ZERO_AUTO_FILE_NAME)
ct_zero_auto_shp <- st_transform(ct_zero_auto_shp, crs = st_crs("+proj=longlat +ellps=GRS80"))
...
writeLines(template.html, file.path(OUTPUT_PATH, paste(paste(OUTPUT_HTML_NAME,currentDate) , ".html", sep = "")))
