---
title: "`r paste(BASE_SCENARIO_NAME, 'vs.', BUILD_SCENARIO_NAME, 'Summary')`"
output:
...
plotly_density_plotter <- function(data_df, index = "one", colors=c("steelblue", "orange"), fill = 'tozeroy',
  ...
  if(percent){
    ...
    p <- switch(index,
                "one" = plot_ly(data=data_df,x = ~xvar, y = ~prop1, colors=colors, color = ~run, fill=fill) %>%
                ...
                "two" = plot_ly(data=data_df,x = ~xvar, y = ~prop2, colors=colors, color = ~run, fill=fill) %>%
                  add_lines(name=~run,alpha=alpha, line = list(shape = shape)) %>%
                  layout(title = ~run,xaxis = list(title=xlabel, tickvals = tickvals, ticktext = ticktext, tickangle = tickangle), yaxis = list(title=ylab, tickformat = ".0%"), showlegend = legend),
                "three" = plot_ly(data=data_df,x = ~xvar, y = ~prop3, colors=colors, color = ~run, fill=fill) %>%
                  add_lines(name=~run,alpha=alpha, line = list(shape = shape)) %>%
                  layout(title = ~run,xaxis = list(title=xlabel, tickvals = tickvals, ticktext = ticktext, tickangle = tickangle), yaxis = list(title=ylab, tickformat = ".0%"), showlegend = legend)
                )
    ...
    p <- switch(index,
                "one" = plot_ly(data=data_df,x = ~xvar, y = ~yvar1, colors=colors, color = ~run, fill=fill) %>%
                  add_lines(name=~run,alpha=alpha, line = list(shape = shape)) %>%
                ...
                "two" = plot_ly(data=data_df,x = ~xvar, y = ~yvar2, colors=colors, color = ~run, fill=fill) %>%
                  add_lines(name=~run,alpha=alpha, line = list(shape = shape)) %>%
                  layout(title = "",xaxis = list(title=xlabel, tickvals = tickvals, ticktext = ticktext, tickangle = tickangle), yaxis = list(title=ylab), showlegend = legend),
                "three" = plot_ly(data=data_df,x = ~xvar, y = ~yvar3, colors=colors, color = ~run, fill=fill) %>%
                  add_lines(name=~run,alpha=alpha, line = list(shape = shape)) %>%
                  layout(title = "",xaxis = list(title=xlabel, tickvals = tickvals, ticktext = ticktext, tickangle = tickangle), yaxis = list(title=ylab), showlegend = legend)
                )
  }
...
Scenario Run Folders as Reference:

Base Scenario: "`r BASE_SUMMARY_DIR`" 

This Scenario: "`r BUILD_SUMMARY_DIR`" 

More information can be found at [SEMCOG's ABM User Guide](https://semcog.github.io/TDM_ActSim/SEMCOG_ActSimV1_User_Guide.html).
...
## Base Highlights {data-width="70"}

### 

```{r Run_Date1_ValueBox}
sample_rate <- ifelse(IS_BASE_SURVEY=="Yes", "Not Applicable", paste("Sample Rate: ", as.character(BASE_SAMPLE_RATE*100), "%", sep = ""))
valueBox(BASE_SCENARIO_NAME, sample_rate, color = "DarkOrange")
...
valueBox(prettyNum(round(base_df$value[base_df$name=="total_population"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Population", icon = "ion-ios-people")
```

### Base Households

```{r Household1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_households"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Households", icon = "glyphicon glyphicon-home")
```

### Base Employment

```{r Employment1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_employment"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Employment", icon = "glyphicon glyphicon-stats-bars")
```

### Base Tours

```{r Tours1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_tours"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Person Tours", icon = "ion-refresh")
```

### Base Trips

```{r Trips1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_trips"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Person Trips", icon = "ion-loop")
```

### Base PMT

```{r PMT1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_pmt"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Person Miles Traveled", icon = "ion-android-walk")
```

### Base Vehicle Trips

```{r VTrips1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_vtrips"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Vehicle Trips", icon = "ion-android-car")
```

### Base VMT

```{r VMT1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_vmt"]/BASE_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Vehilce Miles Traveled", icon = "glyphicon glyphicon-road")
```

## Build Highlights {data-width="70"}
...
valueBox(prettyNum(round(build_df$value[build_df$name=="total_population"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Population", icon = "ion-ios-people")
```

### Build Households

```{r Household2_ValueBox}
valueBox(prettyNum(format(round(build_df$value[build_df$name=="total_households"]/BUILD_SAMPLE_RATE), scientific=F), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Households", icon = "glyphicon glyphicon-home")
```

### Build Employment

```{r Employment2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_employment"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Employment", icon = "glyphicon glyphicon-building")
```

### Build Tours

```{r Tours2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_tours"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Person Tours", icon = "ion-refresh")
```

### Build Trips

```{r Trips2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_trips"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Person Trips", icon = "ion-loop")
```

### Build PMT

```{r PMT2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_pmt"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Person Miles Traveled", icon = "ion-android-walk")
```

### Build Vehicle Trips

```{r VTrips2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_vtrips"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Vehicle Trips", icon = "ion-android-car")
```

### Build VMT

```{r VMT2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_vmt"]/BUILD_SAMPLE_RATE), nsmall = 0, digits = 3, scientific = FALSE, big.mark = ","), "Total Vehicle Miles Traveled", icon = "glyphicon glyphicon-road")
```

## Chart Column 1 {data-width="315"}
...
## Base Highlights2 {data-width="70"}
...
gauge(rate, min = 0, max = 2, abbreviateDecimals = 2, gaugeSectors(danger = c(0,2), colors = c("Green", "Green", "Green")))
```
...
gauge(rate, min = 0, max = 5, abbreviateDecimals = 2, gaugeSectors(danger = c(0,5), colors = c("Green", "Green", "Green")))
```
...
gauge(rate, min = 0, max = 15, abbreviateDecimals = 2, gaugeSectors(danger = c(0,15), colors = c("Green", "Green", "Green")))
```

## Build Highlights2 {data-width="70"}
...
**Mandatory Trip Length Frequency Distribution (TLFD)**
...
htmltools::div(style = "margin:0 auto; padding-left:15px; padding-right:15px;",
  bscols(widths=c(12),
...
# Transit Pass {data-navmenu="Long Term"}

## Description {.sidebar data-width="225"}

**Transit Pass**

The result of the transit pass ownership model showing the overall percentage of transit pass owners by person type. Each bar denotes the percentage share of pass owners within that person type group. 2005 Household Travel Survey data is used as survey target.

**Transit Subsidy**

Result of transit subsidy choice model which chooses if a person has a transit subsidy. Each bar denotes the percentage share of subsidy receivers within that person type group. 2005 Household Travel Survey data is used as survey target.

## Chart Column 1

### Transit Pass Ownership {data-height="300"}

```{r Chart_Transit_Pass_Ownership}
# base_pos <- which(base_csv_names=="percentPassHolders")
# base_df <- base_data[[base_pos]]
# base_df$PERNAME <- person_type_df$name_char[match(base_df$PERTYPE, person_type_df$code)]
# base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
build_pos <- which(build_csv_names=="percentPassHolders")
build_df <- build_data[[build_pos]]
build_df$PERNAME <- person_type_df$name_char[match(build_df$PERTYPE, person_type_df$code)]
build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char[! person_type_char %in% c('Retiree')])

# survey data
if(IS_BASE_SURVEY=="Yes"){
  base_df <- base_data[[which(base_csv_names=="transitPassOwnership")]]
}else{
  base_df <- base_data[[which(base_csv_names=="percentPassHolders")]]
}

base_df$PERNAME <- person_type_df$name_char[match(base_df$PERTYPE, person_type_df$code)]
base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char[! person_type_char %in% c('Retiree')])


std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "PERNAME", y = c("percent_pass_holders"))
p <- plotly_bar_plotter(data = std_DF, xlabel = "Person Type", ylabel = "Percent", ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = F, bottom_offset = 60, tickangle = -30)
p

```

### Transit Subsidy {data-height="300"}

```{r Chart_Transit_Subsidy_Ownership}
# base_pos <- which(base_csv_names=="percentPassHolders")
# base_df <- base_data[[base_pos]]
# base_df$PERNAME <- person_type_df$name_char[match(base_df$PERTYPE, person_type_df$code)]
# base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
build_pos <- which(build_csv_names=="pecentWithSubsidy")
build_df <- build_data[[build_pos]]
build_df$PERNAME <- person_type_df$name_char[match(build_df$PERTYPE, person_type_df$code)]
build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char[! person_type_char %in% c('Retiree')])
...
# survey data
if(IS_BASE_SURVEY=="Yes"){
  base_df <- base_data[[which(base_csv_names=="transitSubsidy")]]
}else{
  base_df <- base_data[[which(base_csv_names=="pecentWithSubsidy")]]
}
...
std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "PERNAME", y = c("percent_with_subsidy"))
p <- plotly_bar_plotter(data = std_DF, xlabel = "Person Type", ylabel = "Percent", ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = F, bottom_offset = 60, tickangle = -30)
p
...
### Transit Pass Discount factors by Perosn Type and Agency {data-height="400"}
...
| Part-time worker        | 0.73 | 0.54  | 0.16  | 0   | 0.6   |
...
### Transit Subsidy Distribution {data-height="200"}

##### 100% subsidy is given to anyone who has been predicted to have subsidy. {data-height="100"}

```{r Chart_Average_Subsidy_Amount}
# base_pos <- which(base_csv_names=="percentPassHolders")
# base_df <- base_data[[base_pos]]
# base_df$PERNAME <- person_type_df$name_char[match(base_df$PERTYPE, person_type_df$code)]
# base_df$PERNAME <- factor(base_df$PERNAME, levels = person_type_char)
# build_pos <- which(build_csv_names=="averageSubsidyAmount")
# build_df <- build_data[[build_pos]]
# build_df$PERNAME <- person_type_df$name_char[match(build_df$PERTYPE, person_type_df$code)]
# build_df$PERNAME <- factor(build_df$PERNAME, levels = person_type_char)
# 
...
# base_df <- build_df
# base_df$average_subsidy_amount <- 0
# 
# std_DF <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "PERNAME", y = c("average_subsidy_amount"))
# p <- plotly_bar_plotter(data = std_DF, xlabel = "Person Type", ylabel = "Percent Discount", ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = F, bottom_offset = 60, tickangle = -30)
# p
...
**Employment vs Workers comparison by Income** 

HH income is used for workers, and dest size term is used to split employment. 

Both are not necessarily accurate and use these plots with caution.
...
```{r job_wrk1a}
knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByCounty.jpeg"), rel_path = FALSE)
# knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByTAZ_County.jpeg"))
```

### By TAZ {data-height="575"}

```{r job_wrk_taz}
 knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByTAZ.jpeg"), rel_path = FALSE)
```
...
#knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByEmpCat_inc1.jpeg"))
knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workers_inc1_total.jpeg"), rel_path = FALSE)

# inc1_plots <- paste0(list.files(BUILD_JPEG_PATH, "inc1_", full.names=TRUE))
# # inc1_plot_names <- str_replace_all(inc1_plots, c("workers_inc1_" = "", BUILD_JPEG_PATH = "", ".jpeg"= ""))
# cat(inc1_plots)
# cat(BUILD_JPEG_PATH)
...
#knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByEmpCat_inc2.jpeg"))
knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workers_inc2_total.jpeg"), rel_path = FALSE)
...
#knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByEmpCat_inc3.jpeg"))
knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workers_inc3_total.jpeg"), rel_path = FALSE)
...
#knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workersByEmpCat_inc4.jpeg"))
knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workers_inc4_total.jpeg"), rel_path = FALSE)
...
knitr::include_graphics(file.path(BUILD_JPEG_PATH, "workers_incAll.jpeg"), rel_path = FALSE)
...
labels <- sprintf(
  "<strong>%s</strong><br/><strong>%s %.2f %s</strong><br/> %s", ct_zero_auto_shp$CnssTrc, ct_zero_auto_shp$txtCmm2, ct_zero_auto_shp$Dff_ZrA, "%", ct_zero_auto_shp$txtCmm1) %>% lapply(htmltools::HTML)
...
*M* : One or more mandatory tours

*N* : No mandatory tours but one or more non-mandatory tours

*H* : No tours (either home all day or out of area)
...
Result of the mandatory tour frequency model, summarized for each person with a daily activity pattern type *M*
...
Results of individual non-mandatory tour frequency model, summarized for each person with a daily activity pattern type *M* or *N*.
...
htmltools::div(style = "margin:0 auto; padding-left:15px",
  bscols(widths=c(3,9),
  list(
...
p1 <- plotly_density_plotter(sd.purpose, index = "one", xlabel = "Miles", percent = T,
                             tickvals = seq(2,41), ticktext = c(seq(1,40), "40pl"), legend=T)
...
*EA*: 3:00 AM to 6:29 AM

*AM*: 6:30 AM to 8:59 AM

*MD*: 9:00 AM to 2:29 PM

*PM*: 2:30 PM to 6:29 PM

*EV*: 6:30 PM to 2:59 AM
...
Out of direction distance is defined as the extra distance to the destination as a result of traveling through the stop location. For stops in the outbound direction, it is based on the distance between the last known location (the tour origin or previous outbound stop) and the tour primary destination. For stops in the inbound direction, it is based on the distance between the last known location (the tour primary destination or previous inbound stop) and the tour origin.
...
htmltools::div(style = "width:97%; margin:0 auto;",
bscols(widths=c(12),
  list(
    filter_select("stopDC", " Select Tour Purpose:", sd.purpose, ~grp_var, multiple=F),
    p1
  )
)
)
...
# t1 <- htmlTable(txtRound(df, digits = 2, excl.cols = 1),
#                 align = "c|r",
#                 rnames = F,
...
sd.purpose <- get_standardDF(data_df1=base_df[!is.na(base_df$tod_agg),], 
                             data_df2=build_df[!is.na(build_df$tod_agg),], 
                             x="tod_agg", y = c("freq_stop", "freq_trip"), grp = "purpose", shared = T)
