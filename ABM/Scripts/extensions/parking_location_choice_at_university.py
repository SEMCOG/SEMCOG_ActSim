...
def closest_parking_zone_xwalk(univ_zones, parking_zones, network_los):
    ...
    closest_zones = pd.Series(dtype=object)
    parking_zones_arr = parking_zones.to_numpy()
    for univ_zone in univ_zones.to_numpy():
        if univ_zone in parking_zones_arr:
            # if zone has parking data, choose that zone
            closest_zones = pd.concat([closest_zones, pd.Series([univ_zone])], ignore_index=True)
        else:
            # find nearest zone from distance skim
            parking_zone_idx = np.argmin(
                skim_dict.lookup(univ_zone, parking_zones_arr, "DIST")
            )
            parking_zone = parking_zones_arr[parking_zone_idx]
            closest_zones = pd.concat([closest_zones, pd.Series([parking_zone])], ignore_index=True)

    closest_parking_df = pd.DataFrame(
        {"univ_zone": univ_zones, "closest_parking_zone": closest_zones.values}
    )
