...
segments = {
    "test": {"TAZ": range(2100,2151)}  # includes University of Michigan Zones
}
...
def file_exists(file_name):
    return os.path.exists(input_path(file_name))
...
if file_exists(MAZ):
    mazs = read_csv(MAZ)
    mazs = mazs[["TAZ", "MAZ"]].sort_values(by="MAZ").reset_index(drop=True)
    mazs = mazs[mazs["MAZ"].isin(land_use["MAZ"])]
    to_csv(mazs, MAZ)

# maz to tap
if file_exists("maz_to_tap_walk.csv"):
...
if file_exists("maz_to_tap_drive.csv"):
...
# maz to maz
if file_exists("maz_to_maz_walk.csv"):
...
if file_exists("maz_to_maz_bike.csv"):
...
# skims
crop_omx("skims", taz.TAZ, num_outfiles=(2 if segment_name == "full" else 1))
